...
def save_gt_volume_beside_tif(src_tif_path: str, vol_norm: np.ndarray, suffix: str = "_gt") -> str:
    """Save normalized GT volume beside the source TIFF and return output path."""
    base, ext = os.path.splitext(src_tif_path)
    out_path = f"{base}{suffix}{ext or '.tif'}"
    tiff.imwrite(out_path, vol_norm.astype(np.float32))
    return out_path
